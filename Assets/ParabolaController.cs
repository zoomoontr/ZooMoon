...
public class ParabolaController : MonoBehaviour
{
	...
	//Debug.Log(Vector3.Distance(parabolaFly.Points[2].position,transform.position)); //Drone ve platform arasındaki uzaklık
		...
		//drone platforma yaklaşınca
		if ( bGrounded==false && Vector3.Distance (parabolaFly.Points [2].position, transform.position) < 900) {

			//Debug.Log("sabitlendi");
			...
			transform.rotation = Quaternion.Slerp (transform.rotation,platform.transform.rotation,animationSpeed*speed);
